...
$sql = "SELECT ProductName, price, ABS(price) AS AbsoluteValue FROM products WHERE price <0;";
...
<h2>Output for UPPER() Function</h2>
<div class=ms-auto><a class='btn btn-outline-success' href='../index.php'>Back to Home</a></div>
</div>


<div class="p-4 rounded-3 mb-4 shadow-sm" style="background-color: #eaedf0;">
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-start'>" . htmlspecialchars($row["price"]) . "</td>";
        echo "<td class='text-center'>" . $row["AbsoluteValue"] . "</td>";
        echo "</tr>";
    }
} else {
