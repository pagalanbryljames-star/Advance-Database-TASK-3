...
$sql = "SELECT ProductName, price, ASIN(0.5) AS AsinValue FROM products LIMIT 5;";
...
<h2>Output for ASIN() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>ASIN(0.5)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["AsinValue"] . "</td>";
        echo "</tr>";
    }
} else {
