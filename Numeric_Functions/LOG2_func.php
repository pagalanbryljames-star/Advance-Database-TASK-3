...
$sql = "SELECT ProductName, price, LOG2(price) AS Log2Value FROM products LIMIT 5;";
...
<h2>Output for LOG2() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>LOG2(Price)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["Log2Value"] . "</td>";
        echo "</tr>";
    }
} else {
