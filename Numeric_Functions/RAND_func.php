...
$sql = "SELECT ProductName, RAND() AS RandomValue FROM products LIMIT 5;";
...
<h2>Output for RAND() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>Random Value</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["RandomValue"] . "</td>";
        echo "</tr>";
    }
} else {
