...
$sql = "SELECT ProductName, price, CEIL(price) AS CeilValue FROM products";
...
<h2>Output for CEIL() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>CEIL(Price)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["CeilValue"] . "</td>";
        echo "</tr>";
    }
} else {
