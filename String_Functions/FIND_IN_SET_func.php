<?php
include '../db.php';
$sql = "SELECT fname,lname,email, FIND_IN_SET('Jessica',fname) as found FROM customers;";
...
<h2>Output for FIND_IN_SET() Function</h2>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $highlight = ($row["found"]>0)? "style ='background-color:yellow;'":"";

        echo "<tr $highlight>";
        echo "<td class='text-center'>" . htmlspecialchars($row["fname"]) . "</td>";
        echo "<td class='text-center'>" . htmlspecialchars($row["lname"]) . "</td>"; 
