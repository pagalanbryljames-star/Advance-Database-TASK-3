...
$sql = "SELECT ProductName, price, TAN(price) AS TanValue FROM products LIMIT 5;";
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>TAN(Price)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["TanValue"] . "</td>";
        echo "</tr>";
    }
} else {
