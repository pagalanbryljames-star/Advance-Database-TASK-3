...
$sql = "SELECT ProductName, price, ACOS(0.5) AS AcosValue FROM products LIMIT 5;";
...
<h2>Output for ACOS() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>ACOS(0.5)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["AcosValue"] . "</td>";
        echo "</tr>";
    }
} else {
