...
$sql = "SELECT 	Description,Celsius, RADIANS(Celsius) AS RadiansValue FROM temperature ;";
...
<h2>Output for RADIANS() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>Celsius</th>
            <th class=text-center>RADIANS(Cesius)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["Celsius"] . "</td>";
        echo "<td class='text-center'>" . $row["RadiansValue"] . "</td>";
        echo "</tr>";
    }
} else {
