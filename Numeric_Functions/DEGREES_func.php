...
$sql = "SELECT ProductName, price, DEGREES(1.5) AS DegreesValue FROM products WHERE ID = 1";
...
<h2>Output for DEGREES() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>DEGREES(1.5)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["DegreesValue"] . "</td>";
        echo "</tr>";
    }
} else {
