...
$sql = "SELECT ProductName, price, ATAN(2.5) AS AtanValue FROM products LIMIT 5;";
...
<h2>Output for ATAN() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>ATAN(2.5)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["AtanValue"] . "</td>";
        echo "</tr>";
    }
} else {
