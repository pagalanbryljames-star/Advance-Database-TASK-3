...
$sql = "SELECT ProductName, price, POW(price, 2) AS PowValue FROM products LIMIT 5;";
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>POW(Price, 2)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["PowValue"] . "</td>";
        echo "</tr>";
    }
} else {
