...
$sql = "SELECT ProductName, price, SIGN(price - 50) AS SignResult FROM products LIMIT 5;";
...
<h2>Output for SIGN() Function</h2>
...
<table class='table table-striped table-hover rounded-3 overflow-hidden shadow-sm'>
    <thead style='background-color:#04AA6D; color:white'>
        <tr>
            ...
            <th class=text-center>SIGN(Price - 50)</th>
        </tr>
    </thead>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='text-center'>" . $row["SignResult"] . "</td>";
        echo "</tr>";
    }
} else {
